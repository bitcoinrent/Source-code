 ...
 * between bitcoinrentd, and bitcoinrent-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("bitcoinrent-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", BITCOINRENT_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", BITCOINRENT_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinrent/bitcoinrent>";
    const std::string URL_WEBSITE = "<https://bitcoinrentcore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitcoinrent-loadblk");
...
/** Sanity checks
 *  Ensure that BitcoinRent is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetBoolArg("-disablefork", false)) {
        printf("NOTE: disabling fork; this node will run on the bitcoinrent network\n");
        FORK_ALLOWED = false;
...
{
    // Make sure only a single BitcoinRent process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BITCOINRENT_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitcoinrent is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitcoinrent is started while in a temporary directory.\n",
    ...
    GenerateBitcoinRents(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE), gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
