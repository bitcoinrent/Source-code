...
static bool ProcessBlockFound(const std::shared_ptr<const CBlock> pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BitcoinRentMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(chainparams, pblock, true, NULL))
        return error("BitcoinRentMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static BitcoinRentMiner(const CChainParams& chainparams)
{
    LogPrintf("BitcoinRentMiner started\n");
    // SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinrent-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in BitcoinRentMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            std::shared_ptr<CBlock> pblock = std::make_shared<CBlock>(pblocktemplate->block);
            IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);

            // LogPrintf("Running BitcoinRentMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("BitcoinRentMiner terminated\n");
        throw;
    } catch (const std::runtime_error& e) {
        LogPrintf("BitcoinRentMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateBitcoinRents(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinRentMiner, boost::cref(chainparams)));
}
