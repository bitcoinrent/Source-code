...
const char* const BITCOINRENT_CONF_FILENAME = "bitcoinrent.conf";
const char* const BITCOINRENT_PID_FILENAME = "bitcoinrentd.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinrentd -foo=bar
...
#else
    const char* pszModule = "bitcoinrent";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinRent
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinRent
    // Mac: ~/Library/Application Support/BitcoinRent
    // Unix: ~/.bitcoinrent
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinRent";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinRent";
#else
    // Unix
    return pathRet / ".bitcoinrent";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINRENT_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINRENT_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure BitcoinRent Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BitcoinRent Core") == std::string::npos) {
