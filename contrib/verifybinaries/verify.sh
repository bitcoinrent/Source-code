...
###   bitcoinrentcore.org and bitcoinrent.org and compares them.
...
WORKINGDIR="/tmp/bitcoinrent_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinrentcore.org"
HOST2="https://bitcoinrent.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinrent-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinrent-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinrent.org failed to provide signature file, but bitcoinrentcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinrent.org and bitcoinrentcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinRent Core binary release signing key installed?"
   fi
